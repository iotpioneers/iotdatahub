...
function SideNavbar() {
  ...
  return (
    <aside
      className={`fixed h-screen bg-white dark:bg-gray-800 shadow-md transition-all duration-300 ease-in-out z-50 ${
              ...
              {links.map((link) => (
                <li key={link.href}>
                  <Link href={link.href}>
                    ...
                    >
                      <div
                        className={`flex-shrink-0 ${isCollapsed ? "w-6 h-6 mx-auto" : "w-5 h-5"}`}
                      >
                        {link.icon}
                      </div>
                      <AnimatePresence>
